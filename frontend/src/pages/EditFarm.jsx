...
import { getFarmById, updateFarm } from "../api/farms";
...
const EditFarm = () => {
  ...
  const fetchFarm = async () => {
    try {
      ...
      setCurrentImageUrl(farm.image_url);
      if (farm.image_url) {
        setImagePreview(getImageUrl(farm.image_url));
      }
    } catch (error) {
      toast.error(error.error || "Failed to fetch farm");
  ...
  const removeImage = () => {
    setImageFile(null);
    setImagePreview(currentImageUrl ? getImageUrl(currentImageUrl) : null);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(error.error || "Failed to update farm");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-2">
          Update farm information, practices, and certifications
        </p>
      </div>
