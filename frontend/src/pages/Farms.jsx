  ...
  const fetchFarms = async () => {
    try {
      ...
      if (filterType === "location" && filterValue) {
        response = await getFarmsByLocation(filterValue, currentPage);
        setFarms(response);
        setTotalPages(1); // Location filter doesn't return pagination
        setTotalFarms(response.length);
      } else if (filterType === "certification" && filterValue) {
        response = await getFarmsByCertification(filterValue, currentPage);
        setFarms(response);
        setTotalPages(1); // Certification filter doesn't return pagination
        setTotalFarms(response.length);
      } else {
        response = await getAllFarms(currentPage);
        ...
        {(filterType !== "all" || searchQuery) && (
          <div className="mt-4 flex items-center gap-2">
            ...
            <span className="text-sm text-gray-600">
              {filterType === "location" && `Filtering by location: ${filterValue}`}
              {filterType === "certification" && `Filtering by certification: ${filterValue}`}
            </span>
          </div>
        )}
      </div>
